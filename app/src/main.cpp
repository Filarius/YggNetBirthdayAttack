...
void logStatistics()
{
    if (totalcount % countsize == 0)
    {
        ...
        std::cout <<
            ...
            "] Filtered: [" << std::setw(3) << countfortune <<
            "] Collisions: [" << std::setw(3) << ducount <<
...
void logKeys(Address raw, const KeysBox keys)
{
    ...
    if (conf.log) // запись в файл
    {
        ...
        output << "Collistions: " << ducount << std::endl;
        output.close();
    }
    mtx.unlock();
}
...
int main(int argc, char *argv[])
{
    ...
    unsigned long long sizec = 1ULL<<(8*4);
    hashmap = new unsigned char[sizec]();
    
    for(unsigned long long j=0; j<sizec-1; j++){
        hashmap[j] = 0;
    }
